...
import { OrderItem, OrderType } from "@/type";
    ...
    { field: "orderId", headerName: "Order ID", width: 110 },
    {
      ...
      renderCell: (params) => (
        <div className="flex gap-2">
          ...
          {params.value.map((item: OrderItem, i: number) => (
            <div key={i} className="flex items-center gap-2 mb-1">
              ...
              />
              <span className="text-sm inline-block mr-2">{item.sku} × {item.quantity} - <span className="text-xs">{item.name} - ({item.personlized || "YOU"})</span></span>
            </div>
          ))}
        </div>
      ),
